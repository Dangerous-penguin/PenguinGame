    ...
    private void AnimateText()
    {
        ...
        for (int i = 0; i < textInfo.characterCount; ++i)
        {
            ...
            for (int j = 0; j < 4; ++j)
            {
                ...
                verts[charInfo.vertexIndex + j] = origin + new Vector3(wave.Evaluate(Random.Range(0, 1f) * speedMultiplier) * textMultiplier,
                    Mathf.Sin(Time.time * 2f + origin.x * 0.01f) * sinMultiplier, 0);
            }
        }
